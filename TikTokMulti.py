...
class TikTok():
    ...
    """
        ...
        if self.userInput == '1':
            self.uid = input(
                '请输入完整的个人主页地址(回车默认，姜胡说：{}):'.format(self.userHomePageExmple))

            self.uid = self.userHomePageExmple if self.uid == "" else self.uid

            self.end = False
            linkFlag = self.judge_link(self.uid, False)
        ...
        # 销毁数据库链接
        self.destroy_db_connetion()

    # 初始化数据库
    def init_db(self):
        con = sqlite3.connect("douyin.db")
        cur = con.cursor()
        cur.execute("create table if not exists video_download(aweme_id text, nickname text, filename text)")
        con.commit()
        con.close()

    # 获取数据库链接游标
    def get_db_cursor(self):
        self.con = sqlite3.connect("douyin.db")
        self.cur = self.con.cursor()

    def destroy_db_connetion(self):
        if self.con:
            self.con.close()

    def check_video_exists(self, aweme_id, nickname):
        self.cur.execute(
            "select * from video_download where aweme_id='{}' and nickname='{}'".format(aweme_id, nickname))
        if self.cur.fetchone():
            return True
        else:
            return False

    def recording_video_2_db(self, aweme_id, nickname, filename=''):
        self.cur.execute(
            "insert into video_download(aweme_id, nickname, filename) values ('{}', '{}', '{}')".format(aweme_id,
                                                                                                        nickname,
                                                                                                        filename))
        self.con.commit()

    def date_formatting_from_GMT(self, time):
        try:
            GMT_FORMAT = '%a, %d %b %Y %H:%M:%S GMT'
            ret_format = '%Y年%m月%d日'
            return (datetime.datetime.strptime(time, GMT_FORMAT) + datetime.timedelta(hours=8)).strftime(ret_format)
    ...
    def photos_download(self, author_name, id, nickname, isUpdateFlag):
        try:
            # 创建并检测下载目录是否存在
            os.makedirs(self.save + '/' + nickname)
        except:
            # 有目录不再创建
            pass
        try:
            ...
            for i in range(len(images)):
                ...
                name = re.sub(r'[\\/:*?"<>|\r\n]+', "_", author_name)
                photoUrl = self.save + '/' + nickname + '/' + name + str(id) + str(i) + '.jpeg'
                photoShortUrl = self.save + '/' + nickname + '/' + self.dealFileName(name) + str(id) + str(
                    i) + '.jpeg'
    ...
    def videos_download(self, author_list, video_list, aweme_id, nickname, max_cursor, userId, isUpdateFlag):
        for i in range(len(video_list)):
            try:
                # 创建并检测下载目录是否存在
                os.makedirs(self.save + '/' + nickname[i])
            except:
                # 有目录不再创建
                pass
                ...
                if self.musicarg == "yes":  # 保留音频
                    ...
                    try:
                        if music.status_code == 200:  # 判断是否响应成功
                            ...
                            m_url = self.save + '/' + nickname[i] + '/' + re.sub(
            ...
            try:
                current_aweme_id, current_nickname = str(aweme_id[i]), nickname[i]
                shortFileName = self.dealFileName(re.sub(r'[\\/:*?"<>|\r\n]+', "_", author_list[i]))

                if self.check_video_exists(current_aweme_id, current_nickname):  # 判断视频是否存在，避免重复下载
                    print("文件已下载，已为你跳过")
                    if isUpdateFlag == True:
                        return
                    continue
                ...
                try:
                    if video.status_code == 200:  # 判断是否响应成功
                        # 文件时间
                        file_modified_date = self.date_formatting_from_GMT(video.headers.get('Last-Modified', ''))
                        v_url_short = self.save + os.path.join(current_nickname, "{}{}.mp4".format(file_modified_date, shortFileName))
                        ...
                        with open(v_url_short, 'wb') as file:  # 显示进度条
                            ...
                            print('\n' + '[下载完成]:耗时: %.2f秒\n' %
                                  (end - start))  # 输出下载用时时间
                        self.recording_video_2_db(current_aweme_id, current_nickname, shortFileName)
                except Exception as error:
                    print(traceback.format_exc())
                    input('下载视频出错!\r')
            except Exception as error:
